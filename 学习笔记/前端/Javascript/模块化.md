## [导出模块的功能](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Modules#导出模块的功能)
...
- 加载一个模块脚本时不需要使用 `defer` 属性 (see [`` attributes](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/script#attributes)) 模块会自动延迟加载。
